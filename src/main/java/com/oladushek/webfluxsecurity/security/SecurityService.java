...
import com.oladushek.webfluxsecurity.exception.AuthException;
...
public class SecurityService {
    ...
    public Mono<TokenDetails> authenticate(String username, String password) {
        return userService.getUserByUsername(username)
                .flatMap(user -> {
                    if (!user.getEnabled()){
                        return Mono.error(new AuthException("Account disabled", "OLAD_USER_ACCOUNT_DISABLED"));
                    }
                    if (!passwordEncoder.matches(password, user.getPassword())){
                        return Mono.error(new AuthException("Invalid password", "OLAD_INVALID_PASSWORD"));
                    }
                ...
                .switchIfEmpty(Mono.error(new AuthException("Invalid username", "OLAD_INVALID_USERNAME")));
    }
}
